...
function deployment_program() {
  ...
  if [ "$(id -u)" -ne 0 ]; then
    ...
    while true; do
      ...
      echo ""
      echo "Copyleft © 2024-2025"
      echo "https://github.com/UnkownFate/docker-tool.sh"
      echo "Last updated: 2025-01-22"
      echo ""
      echo "This work is licensed under GNU AGPL 3.0."
      echo "https://github.com/UnkownFate/docker-tool.sh?tab=AGPL-3.0-1-ov-file"
      echo ""
      echo "Docker 部署程序："
      echo "$project_menu_separator"
      # 显示所有项目（单列）
